...
![Devl Badge](https://img.shields.io/badge/devel%20version-0.1.0-orangered.svg) 
...
[Big Data PE](https://www.bigdata.pe.gov.br) platform, maintained by the
Government of the State of Pernambuco, which is the institution responsible
for the data. To maintain consistency with R package development standards,
all wrapper functions use English names and English parameter names (e.g.,
`bdpe_store_token()`, `bdpe_fetch_data()`, `bdpe_fetch_chunks()`). However,
because the source API is natively in Portuguese, **response column names**
are returned in Portuguese (e.g., `nu_notificacao`, `dt_notificacao`,
...
`"BOA VIAGEM"`, `"SANTO AMARO"`). Additionally, access to the Big Data PE
API requires an approved **access request** on the platform and a connection
to the **PE Conectado** network or a **VPN** — external connections will
time out.
</td></tr></table>
